...
const SCHEMA = [
  'Return ONLY minified JSON (no prose, no backticks) with these keys:',
...
(async () => {
  ...
  if (i < 0 || j < 0) { console.error('No JSON. Raw:\n', txt.slice(0, 2000)); process.exit(1); }
  let a;
  try { a = JSON.parse(txt.slice(i, j + 1)); }
  ...
  if (Array.isArray(a.transit_new) && a.transit_new.length) {
    ...
    s.transit.series = s.transit.series.slice(-10);
  }
